...
import { saveGeneratedSms } from "@/lib/scripts/companion-api";
import {
  SMS_CHAR_LIMIT,
  SMS_DELIMITER,
  type SmsBrief,
...
function fromBlock(values: Record<string, string | number>): SmsVariant {
  const sms = String(values.SMS ?? "").trim();
  return {
...
export function SmsGenerator({ userEmail, isAdmin }: Props) {
  ...
  const onGenerate = async () => {
    if (!topic.trim() || !facts.trim() || !cta.trim()) {
      showToast("Topic, facts, and CTA are required.");
      return;
    }
    setVariants([]);
  ...
  const onCopy = async (idx: number) => {
    ...
    if (!v) return;
    try {
      await navigator.clipboard.writeText(v.sms);
      showToast("Copied");
    } catch {
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-5 pt-6 pb-15">
      <AppShell
        ...
        homeHref="/scripts"
        title="SMS"
        subtitle="Three 160-char blasts in voice"
        printHidden
      />
      <ScriptsTabsNav active="sms" />
        ...
        <section className="rounded-[10px] border-[1.5px] border-line bg-white p-5 shadow-card">
          ...
          <div className="mb-3.5">
            ...
            <textarea
              ...
              rows={4}
              className={`${inputCls} min-h-[90px] resize-y font-dm`}
            />
          </div>

          <div className="mb-3.5">
            <label className={labelCls}>CTA (link or call to action)</label>
            <input
              type="text"
              value={cta}
              onChange={(e) => setCta(e.target.value)}
              placeholder="ditch.co/may18"
              className={inputCls}
            />
          </div>
          ...
          >
            {streaming ? "Generating…" : "Generate SMS Variants"}
          </button>
          {error && (
          ...
          {!showCards && (
            <div className="rounded-[10px] border-[1.5px] border-dashed border-line bg-white px-6 py-12 text-center">
              <p className="m-0 font-bebas text-[20px] tracking-[0.08em] text-muted">
                THREE SMS VARIANTS WILL APPEAR HERE
              </p>
              <p className="mt-2 text-[13px] text-muted">
                Each capped at {SMS_CHAR_LIMIT} characters including the link.
              </p>
            </div>
          )}
              ...
              if (!v) {
                return (
                  ...
                  >
                    ...
                    <div className="h-12 rounded bg-cream" />
                  </div>
                );
              }
              const charCount = v.sms.length;
                ...
                >
                  <div className="mb-2 flex items-baseline justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <h3 className="m-0 font-bebas text-[20px] leading-tight tracking-[0.04em] text-navy">
                        {v.name || (isStream ? "…" : "")}
                      </h3>
                      {v.angle && (
                    ...
                    <div className="text-right">
                      ...
                      <div
                        className={`font-bebas text-[18px] tracking-[0.04em] ${
                          overLimit ? "text-[var(--cat-event)]" : "text-ink"
                        }`}
                      >
                  ...
                  <pre
                    className={`m-0 whitespace-pre-wrap rounded-sm border bg-cream px-3 py-2.5 font-dm text-[13.5px] leading-[1.55] text-ink ${
                      overLimit ? "border-[var(--cat-event)]" : "border-line"
                    }`}
                  >
                    {v.sms}
                    {isStream && v.sms && (
